...
segment-metrics-dry-run-chiba:
	cd $(TRAINING_DIR) && $(TRAINING_PYTHON) -m src.evaluate.segment_metrics_dry_run \
		--config configs/chiba.yaml \
		--metadata ../$(FRONTEND_DIR)/public/metadata/chiba_latest_metadata.json \
		--output outputs/comparisons/chiba_segment_metrics_dry_run.json
