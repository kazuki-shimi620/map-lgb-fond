...
`district_name` を保持する形で首都圏4県を再前処理した結果、町丁目完全一致は19.33%、地区名prefixで町丁目代表点を平均する方式は80.25%、市区町村代表点フォールバックは0.17%だった。市区町村代表点は粗すぎるため、用途地域やハザード判定、近傍施設距離の学習特徴量には使わない。次は町丁目/地区prefix代表点を付与した検証用Parquetを生成し、空間特徴量をdry-run比較する。
