...
| 周辺施設 | `make collect-cinema-osm-national` ほか | `training/data/processed/`、`frontend/public/facilities/nearby_facilities.json` |
| 将来人口 | `make collect-future-population` | rawキャッシュ、地点結合CSV、比較レポート |
| その他の住みやすさ候補 | 個別コマンド | `docs/data-source-acquisition.md` で取得・採用判断を管理 |
...
| JCSCオープンSC一覧 | JCSC公開ページ | 不要 | raw/processedは管理しない | 集計JSONに加え、信頼座標を持つ施設を地図用JSONへ配布する |
| ハザード | 入力JSON/CSVまたは公開データ | 入力元に依存 | raw/processedは管理しない | まずモデル特徴量CSVとして扱い、ブラウザから外部APIを直接呼ばない |
| 映画館・温浴施設・美術館等 | OpenStreetMap、公式施設一覧 | 原則不要 | raw/processedは管理しない | 取得キャッシュとcoverageを保持し、正規化した軽量JSONだけを配布する |
| 将来人口 | 不動産情報ライブラリ XKT013 | `REINFOLIB_API_KEY` | raw/processedは管理しない | dry-runとキャッシュを使って比較し、現行モデルへの採用は見送っている |
