...
初期実装では、`frontend/public/histories/{region}_trend_summary.json` が読み込める場合に、駅別トレンドまたは地域トレンドの年率を使って学習最終年の予測価格を複利で伸ばす。

トレンド集計がない場合は、既存の価格推移履歴から直近の線形増減を推定する fallback を使う。
