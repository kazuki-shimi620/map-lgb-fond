...
直接利用せず、既存ZIPの検証と中断後の再開を行う。

現行モデルは最寄駅名と駅徒歩分を使うため、この2項目を持つCSV/ZIPを学習用の不動産取引データの
主入力にする。不動産価格APIだけでは同等情報を揃えられないため、地区名などで代用しない。
...
地図クリック時は緯度経度と都道府県別の駅マスタJSONから最寄駅を決定する。

検索時は緯度経度を単位球面上の3次元座標へ変換してKD-treeを構築し、最近傍候補を
探索する。KD-treeは読み込んだ駅マスタ配列ごとにキャッシュし、最終距離は
Haversine距離で算出する。全件走査との一致を検証スクリプトで確認する。
...
MVPでは評価時に算出した残差分位点を利用し、以下で算出する。
...
残差分位点はモデルメタデータJSONから読み込む。既存成果物など残差分位点がない場合は、後方互換として `予測価格 ± MAE` を参考価格帯として扱う。
