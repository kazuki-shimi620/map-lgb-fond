...
本ドキュメントは、不動産価格予測アプリで指定地点の災害リスクを取得・正規化し、モデル特徴量として利用するための仕様を定義する。

学習側ではモデル比較可能な特徴量として整備し、フロントエンドでは価格補正と分けた
参考レイヤーとして扱う。2026-07のバックテストでは安定した改善が得られなかったため、
現行の価格モデルには採用していない。
...
ハザード情報は、まず学習用特徴量としてCSV化する。採用可否はベースラインとのバックテストで確認する。
...
地点判定や属性取得には、不動産情報ライブラリAPIなど、公開ハザード情報を取得できるAPIを利用する。具体的なエンドポイントは実装時に確認し、collect処理側へ閉じ込める。
...
ブラウザから直接呼び出さない。APIキーが必要な場合は学習側のcollect処理だけで利用し、rawレスポンス、正規化JSON、学習用CSVを生成する。
...
価格予測は引き続きONNX Runtime Webでブラウザ内実行する。ブラウザに配布する成果物は、モデル採用後にONNXとメタデータへ反映する。

## 4.2 完全静的構成

APIで地点属性を直接取得できない場合は、国土数値情報を事前処理して学習特徴量へ変換する。
...
## 4.3 本リポジトリの実装範囲と採用判断

価格モデルへ投入できるハザード特徴量の生成と、公式ラスタの参考表示を実装済みである。

対象:

* ハザード情報をAPIまたは国土数値情報から取得するcollect処理を作る
* rawレスポンス、正規化JSON、学習用CSVを生成する
* 洪水、土砂災害、津波、高潮のリスクレベル、浸水深、データ有無を数値特徴量へ変換する
* FeatureProvider方式で既存の学習パイプラインへ接続する
* ベースラインとハザード特徴量ありのバックテスト方針を作る
* 重ねるハザードマップのラスタタイル定義を `frontend/public/hazards/layers.json` に配置する
* Leaflet上で確認用のハザードレイヤーを切り替える

行わないこと:

* ブラウザからハザード情報APIを直接呼び出す実装
* APIキーを `frontend/` 配下へ置く実装
* バックテスト前にハザード特徴量を本番モデル成果物へ反映する実装

地点判定は、APIで直接取得できる場合はAPI結果を優先し、難しい場合は国土数値情報をH3またはポイント・イン・ポリゴンで前処理する。

---

## 4.4 地点判定方式の採用判断

初期実装では、学習側collectでAPI取得または国土数値情報前処理を行い、学習用CSVへ集約する方式を採用する。

採用理由:

* ハザード情報をモデル特徴量として使う最低要求に直結する
* APIキーをブラウザへ露出しない
* フロントエンド配布サイズや推論速度に影響させずに検証できる
* バックテストで有効性を確認してからONNX成果物へ反映できる

実装方針:

* 物件データの緯度経度をキーに、ハザード属性を取得または空間結合する
* レスポンスは学習用特徴量に必要な最小列へ正規化する
* 「区域外」と「データなし」は別のフラグとして保持する
* API取得が難しい地域や種別は、国土数値情報のGeoJSON前処理で補完する
...
以下の特徴量を比較したが、首都圏4都県の複数holdoutで安定した改善が得られなかったため、
現行モデルへの採用を見送った。
...
再評価は、データ提供範囲、地点精度、時系列整合のいずれかが大きく改善した場合に限る。
画面では公式ラスタレイヤー、出典リンク、免責を維持し、指定地点の安全性を断定しない。
