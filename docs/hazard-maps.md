...
初期MVPは、ブラウザ推論の軽さとGitHub Pagesでの運用を優先し、価格モデルには接続しない災害リスク表示として実装する。

対象:

* 重ねるハザードマップのラスタタイル定義を `frontend/public/hazards/layers.json` に配置する
* Leaflet上で洪水、土砂災害、津波、高潮のレイヤー表示を切り替える
* 価格予測カードとは別に災害リスクカードを表示する
* 災害リスクカードには、評価状態、出典、免責文、正式ハザードマップ確認の導線を表示する
* 地図クリック時は、価格予測処理を維持しつつ、ハザード評価処理を独立した状態として扱う

初期MVPでは行わないこと:

* 災害リスクによる予測価格の補正
* ONNXモデル、カテゴリ辞書、駅マスタ成果物の再生成
* ブラウザから不動産情報ライブラリAPIを直接呼び出す実装
* APIキーを `frontend/` 配下へ置く実装
* ハザード特徴量を学習済みモデルへ投入する実装

地点判定は次段階で実装する。実装方式は、外部API依存を増やさない静的H3 JSONを第一候補とし、精度やデータサイズが合わない場合にCloudflare Workers中継APIを検討する。

---

## 4.4 地点判定方式の採用判断

初期実装では、静的H3 JSON方式を採用する。

採用理由:

* GitHub Pagesだけで公開する既存構成を維持できる
* APIキーをブラウザへ露出しない
* 推論サーバーや中継APIの運用を増やさない
* ブラウザ内推論と同じく、静的ファイル配信で完結できる
* ハザード特徴量を将来の学習データへ流用しやすい

実装方針:

* 国土数値情報のGeoJSONを事前処理し、都道府県、ハザード種別、H3セル単位へ集計する
* 初期解像度はH3 Resolution 9を基準にし、配布サイズと境界精度を比較する
* レスポンスは `HazardAssessmentResponse` と同じ形に正規化する
* 「区域外」と「データなし」はH3集計時点で区別できるメタデータを保持する
* 境界付近の誤判定が大きい場合は、Resolution 10または元ポリゴン判定へのフォールバックを検討する

Cloudflare Workers中継APIは、以下の場合の代替案として残す。

* 静的JSONが大きくなりすぎ、初回ロードや地域切り替えに影響する
* 最新データの追従頻度を上げる必要がある
* H3集計だけでは地点判定の精度が不足する
* 不動産情報ライブラリAPIの地点判定レスポンスをそのまま利用する方が実装、運用ともに軽い
