...
モデルを配信用ディレクトリへコピーした後、配置済みONNX、メタデータ、カテゴリ辞書の
相対パス、SHA-256、容量を `model-manifest.json` へ自動出力する。3成果物のハッシュから
地域単位の `buildId` を生成するため、組み合わせの不一致を追跡できる。従来の `version` は
...
商業施設の距離特徴量は、取引年ごとに開業済み施設のBallTreeを構築し、物件を一括検索する。
同一座標の施設が複数ある場合、最近傍施設の開業年は従来実装と同じ入力順で決定する。
2026-08-07の1,100件・3候補・3 seedの再計測は14.57秒で、置換前と評価値が一致した。
約68万件の本比較へ進むかは、この実装で中規模サンプルを確認してから判断する。

11,000件の中規模比較は16.10秒で完了したが、規模別8特徴量は従来の空間特徴量より
MAE/RMSEが悪化した。このため全件学習、配信用成果物生成、ONNX更新には進まない。
