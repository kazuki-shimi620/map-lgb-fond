...
* [ ] 不動産情報ライブラリ `XKT004` / `XKT005` / `XKT006` / `XKT007` のAPI仕様、属性名、年度、ジオメトリ種別を確認する
* [ ] `training/src/collect/education_facilities.py` の設計を固め、学校区ポリゴンと施設ポイントの出力形式を分ける
* [ ] 学校・保育園collectorを実装し、小学校、中学校、保育園、幼稚園の正規化CSVを生成する
* [ ] 取引地点または選択地点から最寄学校距離、半径500m/1km件数を生成するFeatureBuilderを実装する
* [ ] 初期はモデル特徴量ではなく参考情報カードとして表示するか判断する
* [ ] 学区、保育園、学校距離を使う場合のデータ取得可否とノイズリスクを確認する

#### 犯罪・治安

* [ ] 都道府県警、市区町村オープンデータ、e-Statの候補を調査し、全国統一可否と粒度差を整理する
* [ ] 犯罪・治安指標は星評価にせず、人口1000人あたり刑法犯認知件数、出典、年度、集計単位を表示する方針を仕様化する
* [ ] `training/src/collect/crime_stats.py` の設計を固め、自治体単位fixtureで正規化CSVスキーマを検証する
* [ ] 人口統計データと結合し、人口あたり犯罪件数を算出するFeatureBuilderを実装するか判断する
* [ ] 犯罪・治安指標を使う場合の出典、粒度、表示上の注意点を確認する

#### 用途地域・都市計画

* [ ] 不動産情報ライブラリ `XKT001` / `XKT002` / `XKT003` のAPI仕様と属性名を確認する
* [ ] `training/src/collect/urban_planning.py` の設計を固め、用途地域ポリゴンを取引地点へ付与する方法を決める
* [ ] 用途地域collectorを実装し、用途地域、建ぺい率、容積率、区域区分の正規化CSVを生成する
* [ ] `FeatureProvider` に用途地域特徴量を追加し、価格モデル特徴量と参考表示の両方で比較する
