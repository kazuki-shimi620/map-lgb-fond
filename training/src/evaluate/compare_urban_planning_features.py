...
class UrbanPlanningCandidate:
    name: str
    urban_planning_features: list[str]
...
CANDIDATES = [
    UrbanPlanningCandidate("baseline", []),
    UrbanPlanningCandidate("urban_planning", URBAN_PLANNING_FEATURES),
    UrbanPlanningCandidate("baseline_no_station", [], include_station=False),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Backtest urban planning features.")
    ...
    report = compare_urban_planning_features(
...
def compare_urban_planning_features(
    *,
    regions: list[str],
    processed_dir: Path,
    ...
    for candidate in CANDIDATES:
        ...
        row["deploymentArtifacts"] = _export_deployment_artifacts(
            candidate=candidate,
            data=data,
            output_dir=output_dir / "urban_planning_feature_models",
        )
        rows.append(row)
    ...
    report = {
        ...
        "matchedRowCount": int(data["has_zoning_data"].sum()),
        "candidates": rows,
    }
    save_json(report, output_dir / "urban_planning_feature_backtest.json")
    (output_dir / "urban_planning_feature_backtest.md").write_text(
...
def feature_lists(candidate: UrbanPlanningCandidate) -> tuple[list[str], list[str]]:
    features = BASE_FEATURES + candidate.urban_planning_features
...
def _model_params(candidate: UrbanPlanningCandidate) -> dict[str, object]:
    return {
...
def render_markdown(report: dict[str, object]) -> str:
    lines = [
        ...
        f"用途地域マッチ件数: {report['matchedRowCount']:,}",
        "",
        "| 候補 | station | 用途地域 | MAE | RMSE | MAPE | ONNX | gzip | 辞書gzip | 学習秒 |",
        "|---|---:|---|---:|---:|---:|---:|---:|---:|---:|",
    ]
