...
CANDIDATES = [
    CommercialCandidate("baseline_2015_window", []),
    CommercialCandidate("city_counts", CITY_COUNT_FEATURES),
    CommercialCandidate("city_counts_and_scale", CITY_COUNT_FEATURES + CITY_SCALE_FEATURES),
    CommercialCandidate(
    ...
    CommercialCandidate(
        "city_counts_scale_no_station",
        CITY_COUNT_FEATURES + CITY_SCALE_FEATURES,
        include_station=False,
    ),
]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Backtest commercial facility features.")
...
def compare_commercial_features(
    *,
    regions: list[str],
    processed_dir: Path,
    facilities_csv: Path,
    ...
    save_json(report, output_dir / "commercial_feature_backtest.json")
    (output_dir / "commercial_feature_backtest.md").write_text(
