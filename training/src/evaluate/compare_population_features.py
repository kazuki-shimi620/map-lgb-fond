...
CANDIDATES = [
    PopulationCandidate("baseline", []),
    PopulationCandidate("population", POPULATION_FEATURES),
    PopulationCandidate("baseline_no_station", [], include_station=False),
    PopulationCandidate("population_no_station", POPULATION_FEATURES, include_station=False),
]


def main() -> int:
    parser = argparse.ArgumentParser(description="Backtest population stats features.")
    ...
    report = compare_population_features(
...
def compare_population_features(
    *,
    regions: list[str],
    processed_dir: Path,
    ...
    population = load_population_stats_csv(population_stats_csv)
    data = add_population_features(data, population)
    ...
    for candidate in CANDIDATES:
        ...
        row["deploymentArtifacts"] = _export_deployment_artifacts(
            candidate=candidate,
            data=data,
            output_dir=output_dir / "population_feature_models",
        )
        rows.append(row)
    ...
    report = {
        ...
        "populationStatsCount": len(population),
        "matchedRowCount": int(data["has_population_data"].sum()),
        "candidates": rows,
    }
    save_json(report, output_dir / "population_feature_backtest.json")
    (output_dir / "population_feature_backtest.md").write_text(
...
def _backtest_candidate(
    *,
    ...
    return {
        "name": candidate.name,
        "features": features,
        "populationFeatures": candidate.population_features,
...
def feature_lists(candidate: PopulationCandidate) -> tuple[list[str], list[str]]:
    features = BASE_FEATURES + candidate.population_features
...
def _model_params(candidate: PopulationCandidate) -> dict[str, object]:
    return {
...
def render_markdown(report: dict[str, object]) -> str:
    lines = [
        ...
        f"人口統計件数: {report['populationStatsCount']:,}",
        f"人口統計特徴量マッチ件数: {report['matchedRowCount']:,}",
        "",
        "| 候補 | station | 人口統計 | MAE | RMSE | MAPE | ONNX | gzip | 辞書gzip | 学習秒 |",
        "|---|---:|---|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in report["candidates"]:
        ...
        lines.append(
            ...
            f"{_features_label(row['populationFeatures'])} | "
