...
from features.urban_planning import (  # noqa: E402
...
class LocationFeatureCandidate:
    name: str
    land_price_features: list[str] = field(default_factory=list)
    urban_planning_features: list[str] = field(default_factory=list)
...
CANDIDATES = [
    LocationFeatureCandidate("baseline"),
    LocationFeatureCandidate("land_price", land_price_features=LAND_PRICE_FEATURES),
    LocationFeatureCandidate(
        "urban_planning", urban_planning_features=URBAN_PLANNING_FEATURES
    ),
    LocationFeatureCandidate(
    ...
    LocationFeatureCandidate("baseline_no_station", include_station=False),
    LocationFeatureCandidate(
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Backtest land price and urban planning feature combinations."
    )
    ...
    report = compare_location_features(
...
def compare_location_features(
    *,
    ...
    land_price_points = load_land_price_points_csv(land_price_points_csv)
    land_price_city_summary = load_land_price_city_summary_csv(
        land_price_city_summary_csv
    )
    urban_planning_areas = load_urban_planning_areas_csv(urban_planning_csv)
    data = add_land_price_features(data, land_price_points, land_price_city_summary)
    data = add_urban_planning_features(data, urban_planning_areas)
    ...
    for candidate in CANDIDATES:
        ...
        row["deploymentArtifacts"] = _export_deployment_artifacts(
            candidate=candidate,
            data=data,
            output_dir=output_dir / "location_feature_models",
        )
        rows.append(row)
    ...
    report = {
        ...
        "landPricePointCount": len(land_price_points),
        "landPriceCitySummaryCsv": str(land_price_city_summary_csv),
        "landPriceCitySummaryCount": len(land_price_city_summary),
        "landPriceMatchedRowCount": int(data["has_land_price_data"].sum()),
        "urbanPlanningCsv": str(urban_planning_csv),
        "urbanPlanningAreaCount": len(urban_planning_areas),
        "urbanPlanningMatchedRowCount": int(data["has_zoning_data"].sum()),
        "candidates": rows,
    }
    save_json(report, output_dir / "location_feature_backtest.json")
    (output_dir / "location_feature_backtest.md").write_text(
...
def _backtest_candidate(
    *,
    ...
    return {
        ...
        "urbanPlanningFeatures": candidate.urban_planning_features,
...
def feature_lists(candidate: LocationFeatureCandidate) -> tuple[list[str], list[str]]:
    features = (
        BASE_FEATURES
        + candidate.land_price_features
        + candidate.urban_planning_features
    )
    categorical_features = list(BASE_CATEGORICAL_FEATURES)
    if candidate.urban_planning_features:
        categorical_features += URBAN_PLANNING_CATEGORICAL_FEATURES
...
def _model_params(candidate: LocationFeatureCandidate) -> dict[str, object]:
    return {
...
def render_markdown(report: dict[str, object]) -> str:
    lines = [
        ...
        f"地価特徴量マッチ件数: {report['landPriceMatchedRowCount']:,}",
        f"用途地域エリア件数: {report['urbanPlanningAreaCount']:,}",
        f"用途地域マッチ件数: {report['urbanPlanningMatchedRowCount']:,}",
        "",
        "| 候補 | station | 地価 | 用途地域 | MAE | RMSE | MAPE | ONNX | gzip | "
        "辞書gzip | 学習秒 |",
        "|---|---:|---|---|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in report["candidates"]:
        ...
        lines.append(
            ...
            f"{_features_label(row['urbanPlanningFeatures'])} | "
