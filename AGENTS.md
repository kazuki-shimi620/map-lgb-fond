...
- ファイル読み込み、検索、差分確認、ビルドログ確認などの軽い調査は、利用環境で選択できる場合は軽量モデルを優先する。
- デプロイ、commit、push、定型コマンド実行など、判断負荷が低い作業も軽量モデルで十分かを先に検討する。
- 設計判断、広範囲のリファクタ、レビュー、障害調査など、誤判断の影響が大きい作業では必要に応じて高性能モデルを使う。
- 長いファイルは必要範囲を `rg` や `sed` で絞って読み、全文読み込みは仕様確認や安全確認に必要な場合だけ行う。

### 高コスト作業の事前計画

大量データの取得・補完・照合、長時間の学習、広範囲の調査など、時間またはトークン消費が大きくなりそうな作業は、いきなり本実行しない。次の順序で進める。

1. `TODO.md` と既存コード、metadata、過去の出力を狭い範囲で確認し、未実施部分だけを特定する。
2. 本実行前に「トークンと時間を抑える方法を調べるフェーズ」を設ける。
3. 全件処理より先に、件数確認、dry-run、少量サンプル、1タイル、1都道府県などの最小単位で成立性を確認する。
4. 想定する入力件数、外部リクエスト数、処理時間、生成物、主なトークン消費要因を概算する。
5. キャッシュ、差分更新、重複排除、再開可能なチェックポイント、スクリプトによる機械処理を検討する。
6. 調査結果、低コスト案、本実行案、残るリスクをユーザーへ提案し、方針を確認してから高コストな本実行へ進む。

特に次のいずれかに当てはまる場合は、事前計画を必須とする。

- 外部APIやWeb検索を多数回実行する。
- PDF、CSV、JSON、ログなどを大量または全件で読み込む。
- 施設名や座標の確認など、同種の手動判断を繰り返す。
- モデルの再学習、全国データ生成、長時間ジョブを実行する。
- 複数の大きなファイルや複数領域をまたぐ変更を行う。

トークン消費を抑える際は、次を優先する。

- LLMで1件ずつ処理せず、まずPythonや既存CLIで抽出・正規化・候補絞り込みを行う。
- 生データ全体ではなく、件数、列名、欠損率、重複率、代表サンプル、エラー行だけを確認する。
- 既に確認済みの行は出典・確認日・信頼度を保存し、再調査しない。
- 長い実行結果はファイルへ保存し、会話には要約と失敗箇所だけを返す。
- 同じ調査を繰り返さないよう、実行条件、途中件数、再開位置をmetadataまたは作業メモへ残す。
- 高コスト作業を小さなバッチに分け、各バッチで品質と費用対効果を確認してから次へ進む。
